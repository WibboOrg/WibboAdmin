<template>
    <div class="flex items-center justify-center w-full" @drop.prevent="dropHandler" @dragover.prevent="inDrag = true" @dragleave="inDrag = false" @dragend.prevent>
        <label class="relative flex flex-col w-full h-16 m-2 overflow-hidden border-4 border-blue-200" :class="inDrag ? 'border-solid animate-pulse' : 'border-dashed'">
            ...
            <div class="flex items-center justify-center w-full h-full" v-else>
                <p class="absolute text-sm tracking-wider text-gray-400 left-2 group-hover:text-gray-600">{{ fileUpload?.name }}</p>
